...
def processPacket(packet, fromSender):
    ...
    if verbose: print("Packet being processed: Type={}, seqnum={}, length={}".format(typ, seqnum, length))
    if typ == 2: # if type == EOT
        if fromSender:
            while not data_buff.empty():
                # delay for longest possible delay and check again.
                delayThread(max_delay)
            if verbose: print("Sending packet: Type={}, seqnum={}, length={}".format(typ, seqnum, length))
        ...
        if not randomTrue(prob_discard):
            ...
            if fromSender:
                if typ == 0:
                    ...
                    pass
                if verbose: print("Adding packet to data buffer: Type={}, seqnum={}, length={}".format(typ, seqnum, length))
                data_buff.put(packet)
            else:
                if typ == 1:
                    raise RuntimeError("Received data from the receiver")
                if verbose: print("Adding packet to ack buffer: Type={}, seqnum={}, length={}".format(typ, seqnum, length))
                ack_buff.put(packet)
            ...
            if verbose: print("Dropped packet: Type={}, seqnum={}, length={}".format(typ, seqnum, length))
...
def delayThread(delay):
    ...
    if verbose: print("Packet delayed by {} milliseconds\n".format(delay))
    time.sleep(s_delay)
